...
public class LogoGeneratorTool {
    ...
    public List<ImageResource> generateLogos(@P("Logo 设计描述，如名称、行业、风格等，尽量详细") String description) {
        ...
        try {
            ...
            log.info("开始生成 Logo, model={}, apiKeyPrefix={}", imageModel,
                    StrUtil.isNotBlank(dashScopeApiKey) ? dashScopeApiKey.substring(0, Math.min(10, dashScopeApiKey.length())) + "..." : "空");
            ...
            log.info("Logo API 返回: usage={}, output={}",
                    result != null ? result.getUsage() : "null",
                    result != null && result.getOutput() != null ? result.getOutput().getResults() : "null");
            if (result != null && result.getOutput() != null && result.getOutput().getResults() != null) {
                List<Map<String, String>> results = result.getOutput().getResults();
                log.info("Logo 生成结果数量: {}", results.size());
                for (Map<String, String> imageResult : results) {
                    String imageUrl = imageResult.get("url");
                    log.info("Logo URL: {}", imageUrl);
